...
hnd_get_resource(coap_context_t *ctx UNUSED_PARAM,
  ...
  coap_add_data(response, strlen(RESOURCE), (unsigned char *)RESOURCE);
...
init_resources(coap_context_t *ctx) {
  ...
  r = coap_resource_init((unsigned char *)endpoint, strlen(endpoint), COAP_RESOURCE_FLAGS_NOTIFY_CON);
...
new_option_node(unsigned short key, unsigned int length, unsigned char *data) {
  ...
  if (strncmp((char *)data, "ep", 2)==0){
    endpoint = coap_malloc( (length-2) * sizeof(char) );
    if (!endpoint) {
      fprintf(stderr, "error malloc\n");
      exit(-1);
    }
    strncpy(endpoint, (char *)data+3, length-3);
    endpoint[length-3] = '\0';
    printf("Endpoint of the registration is: %s\n", endpoint);
  }
    ...
    if (rp->ai_addrlen <= sizeof(addr.addr)) {
      ...
      if (ctx) {
        ...
        if (rp->ai_family == PF_INET6) {
          inet_ntop(AF_INET6, &addr.addr.sin6.sin6_addr, addrstr, sizeof addrstr);
          iport = ntohs(addr.addr.sin6.sin6_port);
        } else {
          inet_ntop(AF_INET, &addr.addr.sin.sin_addr, addrstr, sizeof addrstr);
          iport = ntohs(addr.addr.sin.sin_port);
        }

        printf ("*** Listening at IPv%d address: %s:%i ***\n", rp->ai_family == PF_INET6 ? 6 : 4, addrstr, iport);
